      ...
      <div class="welcome-section mb-8">
        <div class="glass rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div class="quick-actions flex flex-wrap gap-2">
            <Button icon="pi pi-user" class="p-button-rounded p-button-text" tooltip="个人资料" @click="$router.push('/profile')" />
            <Button icon="pi pi-cog" class="p-button-rounded p-button-text" tooltip="设置" @click="$router.push('/settings')" />
            <Button icon="pi pi-bell" class="p-button-rounded p-button-text" :badge="notificationCount.toString()" tooltip="通知" @click="showNotifications = true" />
          </div>
        </div>
      </div>
      ...
      <div class="stats-and-activity grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="recent-activity lg:col-span-2 glass rounded-xl p-6">
          ...
          <div class="activities space-y-4">
            <div v-for="(activity, index) in recentActivities" :key="index" class="activity flex gap-4">
              <div class="activity-icon w-10 h-10 rounded-full flex items-center justify-center" :class="activity.iconBg">
        ...
        <div class="ai-assistant-status glass rounded-xl p-6">
            ...
            <div class="stats space-y-4">
              <div class="stat">
                ...
                <div class="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-green-400 to-cyan-400 rounded-full" :style="{width: `${petEnergy}%`}"></div>
                </div>
              </div>
              ...
              <div class="stat">
                ...
                <div class="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-yellow-400 to-amber-400 rounded-full" :style="{width: `${petMoodLevel}%`}"></div>
                </div>
              </div>
              ...
              <div class="stat">
                ...
                <div class="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div class="h-full bg-gradient-to-r from-pink-400 to-purple-400 rounded-full" :style="{width: `${petIntimacy}%`}"></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="quick-actions">
            ...
            <Button label="AI伴侣空间" icon="pi pi-heart" class="p-button-outlined w-full" @click="$router.push('/ai-companion')" />
          </div>
        </div>
      </div>
    </div>
    ...
    <Sidebar v-model:visible="showNotifications" position="right" class="p-sidebar-md">
      ...
      <div class="notifications p-4 space-y-4">
        ...
        <div
          ...
          :key="notification.id"
          class="notification glass-dark p-4 rounded-lg"
...
<script setup>
import DashboardViews from '@/server/view/DashboardView'
</script>
